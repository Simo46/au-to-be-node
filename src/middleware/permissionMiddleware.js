...
const checkPermission = (action, subjectResolver, conditions = null) => {
  return async (req, res, next) => {
    try {
      ...
      if (!req.user) {
        logger.warn('Controllo permessi fallito: utente non autenticato');
        return next(AppError.authentication('Utente non autenticato'));
      }

      // Determina il soggetto
      let subject;
      if (typeof subjectResolver === 'function') {
        // Se è una funzione, eseguila per ottenere il soggetto
        subject = await subjectResolver(req);
      } else {
        // Altrimenti usa il valore come nome del modello
        subject = subjectResolver;
      }

      // Se non è stato possibile determinare il soggetto
      if (!subject) {
        logger.warn(`Controllo permessi fallito: soggetto non determinato per azione ${action}`);
